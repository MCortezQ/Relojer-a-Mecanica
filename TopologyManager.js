class TopologyManager {
    ...
    // ---> INICIO DETECCIÓN GLOBAL DE COLISIONES <---
    // validateMesh() (arriba) solo chequea interpenetración ENTRE el par que se está
    // mallando. Nada impedía que dos engranajes de ramas distintas del árbol se
    // superpusieran visualmente sin que el sistema lo notara — lo vivimos en la
    // práctica al armar trenes con varias ramas concéntricas.
    // Reutiliza this.system.spatialIndex (ya se reconstruye en cada afterGeometryChange,
    // no crea un índice nuevo). Solo advierte, no invalida nada ni bloquea al usuario.
    checkGlobalCollisions() {
        const system = this.system;
        let warnings = [];
        let gearsWithShaft = system.gears.filter(g => g.shaft);
        if (gearsWithShaft.length < 2 || !system.spatialIndex) return warnings;

        let maxRadius = Math.max(...gearsWithShaft.map(g => g.outsideRadius));
        let checked = new Set();

        for (let gear of gearsWithShaft) {
            // Radio de búsqueda generoso: cualquier par que realmente se solape
            // cumple distancia < r(gear) + r(otro) <= r(gear) + maxRadius.
            let candidates = system.spatialIndex.queryCircle(gear.x, gear.y, gear.outsideRadius + maxRadius);

            for (let candidate of candidates) {
                if (candidate.type !== 'gear') continue;
                let other = candidate.data;
                if (other === gear) continue;
                if (other.shaft === gear.shaft) continue; // mismo eje = compound gear, no es colisión
                // Planos distintos: en un reloj real esto representa profundidad distinta
                // sobre el eje (una rueda más cerca de la platina frontal, otra de la trasera,
                // con su propio puente). Dos engranajes que no comparten plano NUNCA engranan
                // entre sí (TopologyManager.validateMesh ya lo exige para las mallas), así que
                // tampoco deberían poder "chocar": viven a distinta altura, no en el mismo lugar.
                if (gear.plane !== other.plane) continue;
                // Ejes coaxiales (mismo x,y): patrón intencional para agujas concéntricas
                // (minutero/horario/segundero en este mismo proyecto). En este simulador 2D
                // no se modela la profundidad real del arbre, así que dos ejes distintos en
                // el mismo punto NO son una colisión — son capas apiladas a propósito.
                if (Math.hypot(gear.shaft.x - other.shaft.x, gear.shaft.y - other.shaft.y) < 1) continue;
                if (this.meshExists(gear, other)) continue; // ya validado por validateMesh (rootRadius)

                let pairKey = gear.id < other.id ? `${gear.id}-${other.id}` : `${other.id}-${gear.id}`;
                if (checked.has(pairKey)) continue;
                checked.add(pairKey);

                let distance = Math.hypot(gear.x - other.x, gear.y - other.y);
                let minSafeDistance = gear.outsideRadius + other.outsideRadius;

                if (distance < minSafeDistance) {
                    warnings.push({
                        gearA: gear, gearB: other,
                        distance, minSafeDistance,
                        overlap: minSafeDistance - distance
                    });
                }
            }
        }
    ...
    // ---> FIN DETECCIÓN GLOBAL DE COLISIONES <---
